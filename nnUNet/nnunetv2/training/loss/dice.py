...
class TverskyLoss(nn.Module):
    def __init__(self, alpha=0.5, beta=0.5, smooth=1, batch_dice=True, do_bg=True, ddp=False, apply_nonlin=True):
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        tp = (x * y_onehot).sum(axes) if loss_mask is None else (x * y_onehot * loss_mask).sum(axes)
        fp = (x * (1 - y_onehot)).sum(axes) if loss_mask is None else (x * (1 - y_onehot) * loss_mask).sum(axes)
        fn = ((1 - x) * y_onehot).sum(axes) if loss_mask is None else ((1 - x) * y_onehot * loss_mask).sum(axes)
