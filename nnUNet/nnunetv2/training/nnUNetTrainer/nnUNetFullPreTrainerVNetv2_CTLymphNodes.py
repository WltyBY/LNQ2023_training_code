...
class nnUNetFullPreTrainerVNetv2_CTLymphNodes(nnUNetTrainer):
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        """
        return VNet_CCT_dropout_3D(in_channels=1, n_classes=2)

        # return get_network_from_plans(plans_manager, dataset_json, configuration_manager, num_input_channels, deep_supervision=enable_deep_supervision)
    ...
    def _build_loss(self):
        ...
        if self.label_manager.has_regions:
            ...
            print("Use Dice and CE loss")
            loss = DC_and_CE_loss({'batch_dice': self.configuration_manager.batch_dice,
                                   'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, {}, weight_ce=1, weight_dice=1,
                                  ignore_label=self.label_manager.ignore_label, dice_class=MemoryEfficientSoftDiceLoss)
        '''
        print("Use Tversky and CE Loss")
        loss = Tversky_and_CE_loss({'batch_dice': self.configuration_manager.batch_dice, 'alpha': 0.5, 'beta': 0.5,
                                    'smooth': 1., 'do_bg': False, 'ddp': self.is_ddp}, {}, weight_ce=1,
                                   weight_tversky=1,
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        self.optimizer.zero_grad()
        # Autocast is a little bitch. If the device_type is 'cpu' then it's slow as heck and needs to be disabled. If
        # the device_type is 'mps' then it will complain that mps is not implemented, even if enabled=False is set.
        # Whyyyyyyy. (this is why we don't make use of enabled=False) So autocast will only be active if we have a
        # cuda device.
