...
from nnunetv2.training.loss.compound_losses_PED import Tversky_and_BCE_loss
from nnunetv2.training.loss.dice_PED import TverskyDiceLoss
from nnunetv2.training.loss.pretrain_loss_PED import Pretrain_MSE_loss
from nnunetv2.training.nnUNetTrainer.pretrain_utils import *
from nnunetv2.training.VNetv2 import VNet_CCT_dropout_3D


class nnUNetPreTrainer_PED(nnUNetTrainer):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.loss_main = self._build_loss()
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        """
        return VNet_CCT_dropout_3D(in_channels=1, n_classes=1)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        data, target = generate_pair(np.expand_dims(data.numpy(), 1),self.batch_size)
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # del data
            l = self.loss_main(output_main, target) + self.loss_aux(output_aux, target)
    ...
    def validation_step(self, batch: dict, batch_id) -> dict:
        ...
        data, target = generate_pair(data.numpy(), self.batch_size)
        data, target = torch.from_numpy(data).float(), torch.from_numpy(target).float()
        data = data.to(self.device, non_blocking=True)
        ...
        if batch_id == 0:
            z = output_main.shape[2]
            save_path = "/hy-tmp/example_imgs/Epoch{}".format(self.current_epoch)
            if not os.path.isdir(save_path):
                os.mkdir(save_path)
            plt.imsave(os.path.join(save_path, "data.png"), data[0, 0][z//2].cpu(), dpi=300, cmap=plt.cm.gray)
            plt.imsave(os.path.join(save_path, "target.png"), target[0, 0][z//2].cpu(), dpi=300, cmap=plt.cm.gray)
            plt.imsave(os.path.join(save_path, "output_main.png"), output_main[0, 0][z//2].cpu(), dpi=300, cmap=plt.cm.gray)
            plt.imsave(os.path.join(save_path, "output_aux.png"), output_aux[0, 0][z//2].cpu(), dpi=300, cmap=plt.cm.gray)
            print("Saving Example slice(Epoch{}).".format(self.current_epoch))
